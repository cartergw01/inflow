...
## Hybrid briefing: reading clarity before spatial exploration (2026-07-12)

The Observatory remains the product's identity, but `/` now opens a normalized eight-story briefing before asking anyone to decode the map. The top three stories carry publisher excerpts; five more complete the essential queue; the remaining ranked set sits behind an explicit “More from your worlds” action. The universe has a complete DOM story rail that cross-highlights Three.js nodes, making every story keyboard- and screen-reader-accessible while reducing canvas-label collisions. Saved and Sources render independently of WebGL, search is server-backed across story/source/author/topic, and the reader preserves briefing position while adding queue navigation and local typography preferences. No ranking, signal, profile, or database semantics changed.
